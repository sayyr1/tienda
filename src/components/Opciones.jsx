...
const opcionDisplay = opcionesArray.map((opciones) => {
  return (
    <div className='bg-white flex items-center flex-col px-20 shadow-sm'>
      <h1 className="text-2xl md:text-3xl lg:text-xl">{opciones.name}</h1>
      <img className="md:h-40" src={opciones.images} alt={opciones.name} />
      <p className="text-center">{opciones.description}</p>
    </div>
  );
});
